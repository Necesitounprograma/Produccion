...
case '0': // listar usuarios
	...
	if($axbuscar==""){
		...
		$SqlStock = "SELECT TOP 5 ID_INSUMO,NOM_COMERCIAL,NOM_CATEGORIA,P_VENTA FROM INSUMOS_VENDEDORES WHERE NOM_SUBCATEGORIA='PRODUCTO TERMINADO' AND ID_EMPRESA='$axidempresa' ORDER BY NOM_COMERCIAL";

	}else{

		$SqlStock = "SELECT TOP 5 ID_INSUMO,NOM_COMERCIAL,NOM_CATEGORIA,P_VENTA FROM INSUMOS_VENDEDORES WHERE NOM_SUBCATEGORIA='PRODUCTO TERMINADO' AND ID_EMPRESA='$axidempresa' AND NOM_COMERCIAL like '%".$axbuscar."%' ORDER BY NOM_COMERCIAL";			
	}
...
case '2': //BUSCA PEDIDOS PROCESADOS Y LOS CUENTA
	...
	$SqlBuscarPeidos ="SELECT * FROM PEDIDOS_CONTAR WHERE ID_BENEFICIARIO='$axidbeneficiario'AND ID_LOCAL='$axidlocal' AND ESTADO_PEDIDO='PROCESADO'";
...
case '3':
	...
	$SQLGrabar = "INSERT INTO PEDIDOS_CZ (ID_PEDIDO_CZ,FECHA_PEDIDO,HORA_PEDIDO,ID_BENEFICIARIO,ID_LOCAL,COD_USER,ESTADO_PEDIDO,FECHA_ENTREGA,LUGAR_ENTREGA,TIPO_PEDIDO,TIPO_COMPROBANTE,ID_EMPRESA,ANO_PEDIDOS) VALUES ('$axcodmovcz','$axfechapedido','$axhorapedidio','$axidbeneficiario','$axidlocal','$axcodusuario','PENDIENTE','$axfechaentrega','$axlugarentrega','$axtipopedido','$axtipocomprobante','$axidempresa','$axano_actual')";
...
case '6':
	...
	$sql6 ="SELECT * FROM PEDIDOS_DETALLE WHERE ID_LOCAL='$axidlocal' AND ID_PEDIDO_CZ='$axcodmovcz' AND ID_BENEFICIARIO ='$axidbeneficiario' ORDER BY NOM_COMERCIAL ASC";
	//echo $sql6;
...
case '11': //BUSCA PEDIDOS PENDIENTES PARA MOSTRARLO
	...
	$SqlBuscarPeidos ="SELECT * FROM PEDIDOS_CZ WHERE  ID_BENEFICIARIO='$axidbeneficiario'AND ID_LOCAL='$axidlocal' AND ESTADO_PEDIDO='PENDIENTE'";
	$result1=odbc_exec($con,$SqlBuscarPeidos);
...
case '13':
	...
	$sql6 = "SELECT * FROM PEDIDOS_CZ WHERE ID_BENEFICIARIO = '$axidbeneficiario' AND ID_EMPRESA='$axidempresa '";
...
case '14':
	...
	$sql9 = "SELECT TOP 5 * FROM BENEFICIARIO WHERE RUC_BENEF+NOM_PROVEEDOR LIKE  '%".$axbuscarcliente."%' AND TIPO_PROV_CLIE='CLIENTE'";
	//echo "$sql9";
	...
	if(odbc_num_rows($result1) > 0){
		 while ($row=odbc_fetch_array($result1)){
		 	...
		 	$output .='<li id="liclientes" class="li" data-ruccliente='.$row["RUC_BENEF"].' data-idbenef='.$row["ID_BENEFICIARIO"].'>'.$row["RUC_BENEF"].' | '.$row["NOM_PROVEEDOR"].'</li>';
